...
# 섹션 간 이관: 서술이 표로 옮겨가거나 다른 섹션으로 이동한 흔적.
# 우리 파서가 표를 본문에서 제거하므로 '거대한 삭제' 로 잡히지만 정보가
# 사라진 것이 아니라 위치가 바뀐 것이다. 경제적 정보가 아니므로 Phase 5 가
# 반드시 걸러야 한다. (S4 2021 의 <주식매수선택권의 부여 및 행사현황> 이 그 예)
_RE_MOVED = re.compile(
    r"(<표\s*\d*>|\[표\s*\d*\]|상세표\s*참조|참조하시기 바랍니다|"
    r"별첨|아래 표|다음 표와 같|해당 내용은.{0,20}기재)")
...
def classify_paragraph(text: str) -> str:
    ...
    if not t:
        return "기타"
    if _RE_MOVED.search(t) and len(t) <= 120:
        return "섹션 간 이관"
...
def write_taxonomy(cfg: Config, tax: pd.DataFrame, out: Path) -> Path:
    ...
    lines = ["# 남은 기업 간 공통 변경 문단의 4분류\n",
             ...
             "분류 규칙 (우선순위 순)\n",
             "1. **섹션 간 이관** — 표/별첨/상세표 참조 표기. 서술이 표로 옮겨간 흔적이라 "
             "정보 소멸이 아니라 위치 이동이다. 경제적 정보가 아니므로 Phase 5 가 걸러야 한다.",
             "2. **K-IFRS** — 기업회계기준서/한국채택국제회계기준/기준서 제NNNN호/해석서/회계정책",
